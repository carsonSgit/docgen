...
describe("API configuration", () => {
  ...
  it("treats a blank required binding as missing", () => {
    expect(() =>
      createApiDependencies({ ...validEnv, WEB_ORIGIN: "  " }),
    ).toThrow(/WEB_ORIGIN is blank/);
  });

  it("rejects a malformed redirect URI", () => {
    expect(() =>
      createApiDependencies({ ...validEnv, GOOGLE_REDIRECT_URI: "not-a-url" }),
  ...
  it("reports every offending binding at once", () => {
    expect(() => createApiDependencies({})).toThrow(
      /GOOGLE_REDIRECT_URI is not set[\s\S]*WEB_ORIGIN is not set/,
    );
  });
